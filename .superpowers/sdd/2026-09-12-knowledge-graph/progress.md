...
Task 8: whole-branch review APPROVED (reviewer re-ran vitest 102 + pytest 74 clean; Saudi scope audited clean on all 20 SQL strings; loaders idempotent; d3 lifecycle clean). New minors deferred: taxonomy loaded but no taxon nodes yet (dead legend dot; plan narrowed spec §5.1, taxonomy feeds future rollup work); duplicate term node ids when one term linked from two fields (group by term_id alone would fix); catch-all 503 masked a join bug during smoke (consistent with search.ts pattern).
